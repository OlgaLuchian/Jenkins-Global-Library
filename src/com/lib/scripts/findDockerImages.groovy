...
def nexusData = myJsonreader.parse(new URL("https://nexus.devops-chicago.com/service/rest/v1/components?repository=fuchicorp"))
...
while (true) {
    ...
    nexusData = myJsonreader.parse(new URL("https://nexus.devops-chicago.com/service/rest/v1/components?repository=fuchicorp&continuationToken=${token}"))
    nexusData.items.each { if (it.name.contains(branchName)) { versionList.add(it.name + ":" + it.version) } }
    }
